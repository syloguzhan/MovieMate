...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:123456@localhost:5432/movie_mate'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['JWT_SECRET_KEY'] = 'movie-mate-jwt'
...
def fetch_and_save_movies():
    ...
    for page in range(10,80):
        url = f'{BASE_URL1}popular?api_key={API_KEY}&language=en-US&page={page}'
...
def fetch_and_save_tvshows():
    ...
    for page in range(80,100):
        url = f'{BASE_URL2}popular?api_key={API_KEY}&language=en-US&page={page}'
        response = requests.get(url)
...
def get_random_tvshow():
    total_tvshows = TVShow.query.filter(TVShow.vote_count>2000,Movie.imdb_rating>8.0).count()

    random_index = random.randint(0,total_tvshows - 1)

    tvshow = TVShow.query.filter(TVShow.vote_count>2000,Movie.imdb_rating>8).offset(random_index).limit(2).first()
